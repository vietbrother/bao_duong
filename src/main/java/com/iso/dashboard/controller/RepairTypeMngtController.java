...
import com.iso.dashboard.ui.RepairTypeMngtUI;
...
import com.iso.dashboard.view.RepairTypeMngtView;
 ...
 */
public class RepairTypeMngtController {
    
    RepairTypeMngtView view;
    RepairTypeMngtService service;

    CustomGrid pagedTable;
//    String[] headerName = new String[]{"Id", "Username", "Email", "Phone", ""};
    String prefix = "repairTypeMngt.list";//tien to trong file language
    String headerKey = "header.repairTypeMngt";//lay trong file cas
    ...
    String jobListLabel = "repairTypeMngt.list";
    Resource resource;

    public RepairTypeMngtController(RepairTypeMngtView view) {
        this.view = view;
        this.pagedTable = view.getPagedTable();
        initTable(RepairTypeMngtService.getInstance().listRepairTypes(null));
        doAction();
    }

    public void initTable(List<RepairType> lst) {
        ...
        pagedTable.genGrid(container, prefix, headerColumn, null, new HandlerButtonActionGrid() {
            ...
            public void actionEdit(Object obj) {
                RepairType item = (RepairType) obj;
                String uId = String.valueOf(item.getId());
                Notification.show("Edit " + uId);
                RepairType dto = RepairTypeMngtService.getInstance().getRepairTypeById(uId);
                onUpdate(dto);
                        ...
                        new ConfirmDialog.Listener() {
                            ...
                            public void onClose(ConfirmDialog dialog) {
                                if (dialog.isConfirmed()) {
                                    ...
                                    ResultDTO res = RepairTypeMngtService.getInstance().removeRepairType(delId);
    ...
    public void reloadData(List<RepairType> lstUnits) {
        pagedTable.setContainerDataSource(pagedTable.createWrapContainer(createContainer(lstUnits)));
    }

    public IndexedContainer createContainer(List<RepairType> lstUnits) {
...
//        container.addContainerProperty("stt", String.class, null);
    ...
    public boolean validateData(RepairTypeMngtUI ui) {
...
//        }
    ...
    private void onInsert() {
        createDialog(true, new RepairType());
    }

    private void onUpdate(RepairType dto) {
        createDialog(false, dto);
    }

    private void onSearch() {
        List<RepairType> list = RepairTypeMngtService.getInstance().listRepairTypes(view.getTxtRepairType().getValue());
    ...
    private void onExport() {
        ...
        try {
            List<RepairType> lst = RepairTypeMngtService.getInstance().listRepairTypes(view.getTxtRepairType().getValue());
    ...
    private void initDataDialog(RepairTypeMngtUI ui, boolean isInsert, RepairType dto) {
    ...
    public void createDialog(boolean isInsert, RepairType dto) {
        RepairTypeMngtUI ui = new RepairTypeMngtUI(isInsert ? BundleUtils.getString("common.button.add") : BundleUtils.getString("common.button.edit"));
...
//        window.setIcon(VaadinIcons.CALENDAR_USER);
                            ...
                            new ConfirmDialog.Listener() {
                                ...
                                public void onClose(ConfirmDialog dialog) {
                                    if (dialog.isConfirmed()) {
                                        ...
                                        if (isInsert) {
                                            res = RepairTypeMngtService.getInstance().addRepairType(dto);
                                            ComponentUtils.showNotification(BundleUtils.getString("common.button.add") + " " + res.getKey() + " " + res.getMessage());
                                        } else {
                                            res = RepairTypeMngtService.getInstance().updateRepairType(dto);
    ...
    private void getDataFromUI(RepairTypeMngtUI ui, RepairType dto) {
